...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const requiredElements = {
        'create-room-btn': 'Botón crear sala',
        'join-room-btn': 'Botón unirse',
        'room-form': 'Formulario de sala',
        'form-title': 'Título del formulario',
        'room-config-form': 'Formulario de configuración',
        'create-options': 'Opciones de crear',
        'join-options': 'Opciones de unirse',
        'cancel-btn': 'Botón cancelar'
    };

    for (const [id, name] of Object.entries(requiredElements)) {
        const element = document.getElementById(id);
        if (!element) {
            console.error(`ERROR: No se encontró el elemento ${name} con ID: ${id}`);
            return;
        }
    }
...
// CORRECCIÓN: Event listener del formulario (SIN DUPLICADOS)
roomConfigForm.addEventListener('submit', (e) => {
    ...
    if (mode === 'create') {
        ...
        const textChat = document.getElementById('text-chat')?.checked;
        const voiceChat = document.getElementById('voice-chat')?.checked;
        
        if (!gameMode || isNaN(numPlayers)) {
            console.error('Datos de sala inválidos');
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
        ...
        if (!roomId) {
            console.error('ID de sala requerido');
            if (typeof Swal !== 'undefined') {
                Swal.fire({
                    icon: 'error',
                    title: 'Error',
                    text: 'Por favor ingresa el ID de la sala'
                });
            } else {
        ...
        console.error('Modo no definido:', mode);
        alert('Error: No se ha seleccionado una opción válida');
    }
});
            ...
            setTimeout(() => {
                ...
                if (teamSetup && !document.getElementById('start-btn')) {
                    ...
                    btn.addEventListener('click', () => {
                        ...
                        console.log('Enviando equipos:', teams);
    ...
    socket.on('roomError', (msg) => {
        ...
        if (typeof Swal !== 'undefined') {
            Swal.fire({
                ...
                // Resetear el formulario y modo
                if (roomConfigForm) {
                    roomConfigForm.reset();
                }
                mode = null;
                
                // Mostrar los botones principales nuevamente
                if (createBtn && joinBtn && roomFormDiv) {
    ...
    socket.on('playerEliminated', (data) => {
        // No intentar usar voiceChat si el socket está desconectado
        if (socket.connected && (window._gameMode === 'teams' || window._gameMode === 'ctf') && voiceChat.enabled) {
            setVoiceChatMode(true, 3000);
        }
        
        let message = data.eliminatedBy ? `Eliminado por ${data.eliminatedBy}` : 'Cayó en un hoyo';
        if (data.hadFlag) {
            message += '\n¡La bandera ha vuelto a su base!';
        }
    ...
    socket.on('disconnect', (reason) => {
        ...
        if (reason === 'io server disconnect') {
            // Reconectar automáticamente si el servidor nos desconectó
            socket.connect();
        }
    });
...
function mostrarConfiguracionEquipos(roomInfo) {
    ...
    document.body.innerHTML = `
    <div id="team-setup" style="padding: 20px; text-align: center; background: linear-gradient(135deg, #0d5017 0%, #1a3a1f 50%, #0d5017 100%); min-height: 100vh; color: white;">
        <h2 style="color: #4CAF50;">Configuración de Equipos</h2>
        <p style="font-size: 1.2em;">ID de sala: <b style="color: #4CAF50;">${roomInfo.roomId}</b></p>
        ${faltan > 0 ? `<p style="color: #ffaa00;">Esperando ${faltan} jugador${faltan === 1 ? '' : 'es'} más...</p>` : ''}
        
        <div id="chat-container" style="margin: 20px auto; width: 80%; max-width: 800px;">
            <div id="chat-messages" style="height: 150px; overflow-y: auto; background: #1a1a1a; padding: 10px; border-radius: 5px;"></div>
            <div style="display: flex; margin-top: 10px;">
                <input type="text" id="chat-input" placeholder="Escribe un mensaje..." style="flex: 1; padding: 8px; border: 1px solid #333; border-radius: 3px; background: #222; color: white;">
        ...
        <div style="display: flex; justify-content: space-around; margin: 20px 0; align-items: flex-start;">
            <div id="team-a-container" style="border: 3px solid #ff4444; border-radius: 10px; padding: 15px; min-height: 200px; width: 200px; background: rgba(0,0,0,0.5);">
                <h3 style="color: #ff4444; margin: 0 0 15px 0;">Equipo A</h3>
                <div id="team-a" class="team-zone" data-team="A" style="min-height: 150px;"></div>
            </div>
            
            <div id="unassigned-container" style="border: 2px dashed #666; border-radius: 10px; padding: 15px; min-height: 200px; width: 200px; background: rgba(0,0,0,0.5);">
            ...
            <div id="team-b-container" style="border: 3px solid #4444ff; border-radius: 10px; padding: 15px; min-height: 200px; width: 200px; background: rgba(0,0,0,0.5);">
        ...
        ${isHost ? `
            ...
            ">
                ...
                " disabled>Comenzar Juego</button>
                <p id="teams-status" style="color: #aaa; margin-top: 10px; font-size: 14px;">
                    Esperando que los jugadores elijan equipo...
                </p>
            </div>
        ` : ''}
    </div>
    ...
    function setupDragAndDrop() {
        ...
        document.addEventListener('dragstart', (e) => {
            // Solo permitir arrastrar si es la tarjeta del usuario actual
            if (e.target.classList.contains('player-card') && e.target.dataset.playerId === window._mySocketId) {
                draggedElement = e.target;
                e.target.style.opacity = '0.5';
                e.dataTransfer.effectAllowed = 'move';
            } else if (e.target.classList.contains('player-card')) {
                // Prevenir arrastrar tarjetas de otros jugadores
                e.preventDefault();
            }
        });
        ...
        document.addEventListener('dragover', (e) => {
            if (e.target.classList.contains('team-zone') || e.target.closest('.team-zone')) {
                ...
                if (draggedElement?.dataset.playerId === window._mySocketId) {
        ...
        document.addEventListener('drop', (e) => {
            if (e.target.classList.contains('team-zone') || e.target.closest('.team-zone')) {
                ...
                if (draggedElement && draggedElement.dataset.playerId === window._mySocketId) {
                    const team = zone.dataset.team;
                    const teamNumber = team === 'A' ? 0 : (team === 'B' ? 1 : null);
                    
                    // Mostrar indicador visual de que está procesando
                    draggedElement.style.opacity = '0.5';
                    
                    console.log('Moviendo jugador al equipo:', {
                        team: teamNumber,
                        playerId: window._mySocketId,
                        roomId: window._roomId
                    });
                    
                    // Mover la tarjeta inmediatamente a la zona correspondiente
                    if (teamNumber === 0) {
                        document.getElementById('team-a').appendChild(draggedElement);
                    } else if (teamNumber === 1) {
                        document.getElementById('team-b').appendChild(draggedElement);
                    } else {
                        document.getElementById('unassigned').appendChild(draggedElement);
                    }
                    
                    // Actualizar el estilo de la tarjeta
                    if (teamNumber === 0) {
                    ...
                    // Emitir evento al servidor para actualizar el equipo
                    ...
                    // Restaurar opacidad normal después de un momento
                    setTimeout(() => {
                        if (draggedElement) {
    ...
    // Manejador de actualizaciones de equipo
    socket.on('teamUpdate', (data) => {
        console.log('Recibida actualización de equipo:', data);
        const users = data.users || [];
        
        // Obtener las zonas de equipo
        const unassignedZone = document.getElementById('unassigned');
        const teamAZone = document.getElementById('team-a');
        const teamBZone = document.getElementById('team-b');
        
        if (!unassignedZone || !teamAZone || !teamBZone) {
            console.error('No se encontraron las zonas de equipo');
            return;
        }

        // Primero limpiar todas las zonas
        unassignedZone.innerHTML = '';
        teamAZone.innerHTML = '';
        teamBZone.innerHTML = '';
        
        // Crear todas las tarjetas nuevamente con la información actualizada
        users.forEach(user => {
            const card = document.createElement('div');
            card.className = 'player-card';
            card.setAttribute('draggable', user.id === window._mySocketId ? 'true' : 'false');
            card.dataset.playerId = user.id;
            
            // Agregar un indicador del equipo actual
            let teamIndicator = '';
            if (user.team === 0) {
                teamIndicator = '🔴';
            } else if (user.team === 1) {
            ...
            card.style.cssText = `
                ...
                margin: 5px;
                border-radius: 5px;
                cursor: ${user.id === window._mySocketId ? 'move' : 'default'};
                user-select: none;
                font-weight: bold;
                transition: all 0.3s ease;
                position: relative;
                ${user.id === window._mySocketId ? 'box-shadow: 0 0 10px rgba(255,255,255,0.3);' : ''}
            `;
        
            // Actualizar la posición de todas las tarjetas de jugador
        users.forEach(user => {
            let card = document.querySelector(`[data-player-id="${user.id}"]`);
            ...
            // Marcar esta tarjeta como actualizada
            updatedCards.add(user.id);
            
            // Aplicar estilos según el equipo
            if (user.team === 0) {
                card.style.border = '2px solid #ff4444';
                card.style.background = '#4a1f1f';
                teamAZone.appendChild(card);
                
                // Agregar efecto de brillo para equipo A
                card.style.boxShadow += ', 0 0 15px rgba(255,68,68,0.3)';
                
            } else if (user.team === 1) {
                card.style.border = '2px solid #4444ff';
                card.style.background = '#1f1f4a';
                teamBZone.appendChild(card);
                
                // Agregar efecto de brillo para equipo B
                card.style.boxShadow += ', 0 0 15px rgba(68,68,255,0.3)';
                
            } else {
                card.style.border = '2px solid #666';
                card.style.background = '#333';
                unassignedZone.appendChild(card);
            }
            ...
            // Aplicar los nuevos estilos con animación
            Object.assign(card.style, newStyle);
            
            // Mover la tarjeta a la zona correcta si no está ahí
            if (card.parentElement !== targetZone) {
                // Añadir efecto visual al mover
                card.style.opacity = '0.5';
                targetZone.appendChild(card);
                
                // Restaurar opacidad y escala después de la transición
                setTimeout(() => {
                    card.style.opacity = '1';
                    card.style.transform = 'scale(1)';
                }, 50);
            ...
            // Si es mi tarjeta, actualizar visual especial
            if (user.id === window._mySocketId) {
                card.style.boxShadow = '0 0 10px rgba(255,255,255,0.3)';
                card.style.fontWeight = 'bold';
            }
        });        // Actualizar el estado del botón de comenzar si es el host
        if (socket.id === window._hostId) {
            const startButton = document.getElementById('start-game-button');
            const statusText = document.getElementById('teams-status');
            if (!startButton || !statusText) return;
            
            const teamA = users.filter(u => u.team === 0).length;
            const teamB = users.filter(u => u.team === 1).length;
            const unassigned = users.filter(u => u.team === null).length;
            
            const isBalanced = teamA === teamB;
            const allAssigned = unassigned === 0;
            const canStart = isBalanced && allAssigned && teamA > 0;
            
            startButton.style.display = 'inline-block';
            startButton.disabled = !canStart;
            startButton.style.opacity = canStart ? '1' : '0.5';
            
            statusText.textContent = canStart 
                ? '¡Equipos equilibrados! Puedes comenzar el juego.'
                : `Esperando jugadores... (A: ${teamA}, B: ${teamB}, Sin equipo: ${unassigned})`;
            statusText.style.color = canStart ? '#00ff00' : '#aaa';
        }
    });
    ...
    if (teamADiv && data.bolas) {
        ...
        teamADiv.innerHTML = teamAPlayers.map(player => `
            ...
            ">
                ${player.username}
                ${player.alive ? '●' : '💀'}
                ${player.id === window._mySocketId ? ' (Tú)' : ''}
            </div>
        `).join('');
    }
    
    if (teamBDiv && data.bolas) {
        const teamBPlayers = data.bolas.filter(p => p.team === 1);
        teamBDiv.innerHTML = teamBPlayers.map(player => `
            <div style="
                background: #1f1f4a; 
                color: white; 
                padding: 8px; 
